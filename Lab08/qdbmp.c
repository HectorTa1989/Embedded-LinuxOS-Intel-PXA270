...
struct _BMP
{
	BMP_Header	Header;    /* this member is the Object of struct above _BMP_Header */
	UCHAR*		Palette;
	UCHAR*		Data;
};
...
BMP* BMP_Create( UINT width, UINT height, USHORT depth )
{
	...
	bmp->Header.ImageDataSize		= bytes_per_row * height;        /* E.g: a 4×2 pixel bitmap, with 32 bits/pixel (4 bytes/pixel) encoding would have Size of the raw bitmap data= 4*2*4= 32 bytes */
